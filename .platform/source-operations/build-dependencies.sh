...
with open('.platform.app.yaml', 'r') as file:
    data = yaml.safe_load(file)
    current = data['dependencies'][sys.argv[1]][sys.argv[2]]
    if (latest > current):
        data['dependencies'][sys.argv[1]][sys.argv[2]] = latest
        with open('.platform.app.yaml', 'w') as file:
